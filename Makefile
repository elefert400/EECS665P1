...
# runs its action (which is to call "make lakec")
# ---------------------------------------------
all: 
	make lakec
...
# single binary named lakec (the -o argument). 
# The $@ is an automatic variable that takes on whatever the 
# target of the rule is. So, in this case, since the target
# of the rule is lakec, $@ resolves to lakec. It often handy
# to have an action write to $@ since that is the 
# file that we are trying to prepare in a given rule. 
# ---------------------------------------------
lakec: $(OBJ_SRCS)
...
# compiler. Note here the difference with the lakec rule:
...
# lake.l (hence lake.l is the dependency of lexer.yy.cc). 
...
# Run the program on test1.lake, writing the
...
# ---------------------------------------------
test: all
	./lakec test1.lake test1.out 2> test1.err
...
# ---------------------------------------------
cleantest:
	rm -f test1.out test1.err *.o lakec
